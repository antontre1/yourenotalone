...
urls.each do |url|
  ...
  new_user = User.new(email: "monmail#{i}@a.com",
                      ...
                      created_at: DateTime.now - (200..2000).to_a.sample.hours)
  file = URI.open(url)
...
# Create 4 theme
    ...
    Theme1 = Theme.new(title: "Découverte du Lupus", description: "Toutes les questions que peuvent se poser ceux et celles qui découvrent le Lupus", created_at: DateTime.now - (1000..2000).to_a.sample.hours)
    Theme1.save

    Article1 = Article.new(title: "Comment reconnaitre une poussée de Lupus ?", content: 'Toutes les questions que peuvent se poser ceux et celles qui découvrent le Lupus', user_id: User.all.sample.id, created_at: DateTime.now - (0..1000).to_a.sample.hours)
    ...
    puts Article1.title
    Article1.save!
    Article2 = Article.new(title: "Quels sont les douleurs du Lupus ? ", content: 'Toutes les questions que peuvent se poser ceux et celles qui découvrent le Lupus', user_id: User.all.sample.id, created_at: DateTime.now - (0..1000).to_a.sample.hours)
    ...
    Article2.save!
    Article3 = Article.new(title: "Quelles sont les causes du Lupus", content: 'Toutes les questions que peuvent se poser ceux et celles qui découvrent le Lupus', user_id: User.all.sample.id, created_at: DateTime.now - (0..1000).to_a.sample.hours)
  ...
  Theme2 = Theme.new(title: "Alimentation", description: "Posts relatifs à l'alimentation et comment se nourrir avec le Lupus", created_at: DateTime.now - (1000..2000).to_a.sample.hours)
  Theme2.save
...
', user_id: User.all.sample.id, created_at: DateTime.now - (0..1000).to_a.sample.hours)
...
- La luzerne et les germes de haricots mungo (souvent appelés erronément pousses ou germes de soja), qui pourraient provoquer des poussées de lupus.', user_id: User.all.sample.id, created_at: DateTime.now - (0..1000).to_a.sample.hours)
    ...
    Article5 = Article.new(title: "L’ail : Très mauvaise idée donc évitez ! ", content: 'J’ai été mangé chez ma Grand-Mère il y a 2 jours, et j’ai tout de suite senti après le repas que je faisais une poussée. J’avais déjà eu la même réaction 2 fois par le passé. Et j’ai tout de suite fat la connexion : les 3 plats contenaient de l’ail.
      Je sais que l’ail peut relever vos plats les plus fades, mais évitez pour votre propre bien.', user_id: User.all.sample.id, created_at: DateTime.now - (0..1000).to_a.sample.hours)
    ...
    Article5.save!
  Theme3 = Theme.new(title: "Ma peau et le Lupus", description: "Comment gérer ma peau avec un Lupus cutané", created_at: DateTime.now - (1000..2000).to_a.sample.hours)
  Theme3.save
  Theme4 = Theme.new(title: "Vie de Couple", description: "Comment concilier vie de couple et Lupus", created_at: DateTime.now - (1000..2000).to_a.sample.hours)
  Theme4.save
    Article1 = Article.new(title: "Comment j'ai annoncé à mon conjoint que je souffre de Lupus", content: 'Je souffre du Lupus depuis près de 4 ans et je n’avais jamais vraiment vécu une relation depuis que j’ai été diagnostiquée.
      ...
      Je suis tellement heureuse !!!!', user_id: User.all.sample.id, created_at: DateTime.now - (0..1000).to_a.sample.hours)
